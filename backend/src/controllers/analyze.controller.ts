...
export const makeAnalyzeController =
  (vision: IVisionService, mime: IMimeValidator) =>
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!file) {
        ...
        err.status = 400;
        err.publicMessage = 'No image provided';
        throw err;
      }
      ...
      if (!ok) {
        ...
        err.status = 400;
        err.publicMessage = 'Only image files are allowed';
        throw err;
      }
